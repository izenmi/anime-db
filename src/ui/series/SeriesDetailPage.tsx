...
/** シリーズの作品を放送順(古い順)固定で表示する。「シリーズを追う順」を示すのが
...
export function SeriesDetailPage() {
  ...
  useSeo({
    title: series?.name,
    description: series
      ? `${series.name}の収録${series.workCount}作品を放送順に紹介。${series.description ?? ""}`.slice(0, 160)
      : undefined,
    jsonLd: series
      ? [
          breadcrumbJsonLd([
    ...
    <div className="page">
      ...
      {state.status === "ready" && state.data && (
        <>
          ...
          <p className="page-subtitle">{state.data.workCount}作品(放送順)</p>
