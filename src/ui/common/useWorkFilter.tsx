...
import { ORIGINAL_TYPE_LABEL, seasonSortKey } from "./labels";

/**
 * 作品リストを持つページ(テーマ詳細・スタッフ/スタジオ/声優詳細・アワード詳細)で
 ...
 *
 * WorkListPage 側は「スタジオ」など、そのページでしか意味のない条件も持つため統合していない。
 * ここに置くのは**どの詳細ページでも意味がある条件だけ**にしている。
 */
 ...
 *  制作者名のフィールド名はサイトごとに違うので、存在するものだけを拾う。
 *  姉妹サイトへ同じフックを移植できるようにするため。 */
export function matchesKeyword(w: WorkGenerated, keyword: string) {
  if (!keyword) return true;
  const w2 = w as unknown as Record<string, unknown>;
  const names = ["directorNames", "studioNames", "authorNames", "originalAuthorNames"]
...
export function themeOptionsOf(works: WorkGenerated[] | undefined, exclude?: string) {
  ...
  for (const w of works ?? []) {
    // ネタバレテーマは WorkCard 側で伏せているので、選択肢にも出さない
    const hidden = new Set(w.spoilerThemeIds);
    w.themeIds.forEach((id, i) => {
      if (id === exclude || hidden.has(id)) return;
...
export function useWorkFilter(works: WorkGenerated[] | undefined, defaultSort = "year-desc") {
  ...
  const hasActiveFilters = Boolean(q || originalType || theme);
    ...
    <div className="filter-row">
      ...
      {hasActiveFilters && (
        <button
          ...
          onClick={() => {
            const next = new URLSearchParams(params);
            ["q", "originalType", "theme"].forEach((k) => next.delete(k));
            setParams(next, { replace: true });
          }}
        >
          フィルターをクリア
