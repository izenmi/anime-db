 ...
 *
 *  1シードあたりの式はビルド側 relatedIdsFor と同じ: テーマIDFコサイン + 同スタジオ+0.15 +
...
function scoreBySeeds(works: WorkGenerated[], seeds: WorkGenerated[]) {
  ...
  const seedData = seeds.map((s) => ({
    ...
    norm: normOf(s),
    studios: new Set(s.studioIds),
    directors: new Set(s.directorIds),
  }));
  const seedIds = new Set(seeds.map((s) => s.id));

  const scored: { work: WorkGenerated; score: number }[] = [];
  for (const w of works) {
    // 除外はシード自身だけ。同スタジオ・同監督の別作品が上位に来るのは詳細ページと同じ望ましい挙動。
    ...
    for (const sd of seedData) {
      ...
      if (w.studioIds.some((id) => sd.studios.has(id))) score += SAME_STUDIO_BONUS;
      if (w.directorIds.some((id) => sd.directors.has(id))) score += SAME_DIRECTOR_BONUS;
      total += score;
    }
    ...
    <>
      ...
      <div className="filter-row">
        <input
          type="search"
          value={q}
          placeholder="タイトル・スタッフで作品を検索"
          aria-label="タイトル・スタッフで作品を検索"
        ...
        <>
          ...
          {results.length > 0 && (
            <p className="page-subtitle">
              ...
              同じスタジオ・監督の作品には加点しているため、100%を上限に丸めています。
            </p>
          )}
        </>
      )}
