...
TVアニメ・劇場アニメ・OVA・ONAを制作スタジオ・監督・シリーズ構成・声優・放送クール・受賞歴・テーマから検索できるファンデータベース。姉妹サイト6番目(2026-08-08作成)。**年表(/timeline)機能はユーザー指示で2026-08-09に削除済み(再提案しない)**。scaffoldのコピー元は**mystery-db**(人物系エンティティが最多で改造距離が最短だったため)。アーキテクチャ・デザインシステム・運用ノウハウはranobe-db系列をそのまま踏襲している。
...
- **3177作品(2026-08-09にOVA・ONAまで収録範囲を広げて244作品を追加)**。TV2633・劇場300・ONA153・OVA91。スタッフ1418・スタジオ437・声優2117
  - **成人向け作品は登録しない**。AniListの人気順にはCG集原作のOVA等が多数含まれるので、stage の一覧を目視して `drop` で外す(判定は自動化していない)
  - **`format` は tv/movie/ova/ona の4値**。`TV_SHORT` は尺が短いだけのTV放送なので tv に寄せる。AniListが `ONA` としている作品でも実際はTV放送のことがある(『アオのハコ』)
...
好きなテーマを最大3つ選ぶと、テーマの重なりが近い作品を一致度つきで20作品出す1ページ
(`src/ui/recommend/RecommendPage.tsx`)。

- **スコアは「この作品が好きなら」と同じ式**をクライアント側で再現している(`idf(t)=log(N/df(t))` の
  IDF重み付きコサイン類似度)。**別の式を新造しないこと** — 詳細ページの並びと食い違うため。
  制作者ボーナスは種になる作品がないので足さず、そのぶん上限が1.0になり「一致度◯%」として出せる
- **同点が支配的になる**(選んだテーマだけを持つ作品は全部100%)。id昇順だけだと
  スラッグ順という無意味な並びになるので、期数→受賞件数→id昇順で崩している
- **データは `generated/recommend-index.json`**(テーマ表 + 各作品のテーマidだけ)。
  これだけで選択UIが動き、**works.json はテーマを1つ選んでから**取りに行く。
  **この索引の読み手は /recommend だけ。ページを消すなら生成も消すこと**。spoilerテーマ2件は選択肢からも集計からも外している
...
- **ネタバレテーマは `visibleThemes()` で必ず外す**(df・norm・dot・ラベルのすべて)。ビルド側
  ...
  「一致したテーマ」のラベルからネタバレが漏れる(2026-08-13時点で spoiler 付きテーマを持つ
  作品は0件だが、機構は将来のために入れてある)
...
- **ギャラリー表示(`?view=gallery`)はこのサイトには入れていない**(表紙を大判で眺める価値は
  ...
  Staff/Studio/VoiceActor の各詳細が `PersonDetailPage` に共通化されておらず3ファイル個別対応になる)
