...
TVアニメ・劇場アニメを制作スタジオ・監督・シリーズ構成・声優・放送クール・受賞歴・テーマから検索できるファンデータベース。姉妹サイト6番目(2026-08-08作成)。scaffoldのコピー元は**mystery-db**(人物系エンティティが最多で改造距離が最短だったため)。アーキテクチャ・デザインシステム・運用ノウハウはranobe-db系列をそのまま踏襲している。
...
- **`season` はシリーズ第1期の放送開始クール** `{ year, quarter: winter|spring|summer|fall }`。埋め込みフィールドで、エンティティ化していない(game-dbの`platforms`と同じ考え方)。`/timeline`が年×クールの年表を描く
