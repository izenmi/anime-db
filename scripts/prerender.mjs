...
// every work/staff/studio/voice-actor/theme/award page instead of one empty shell for
...
const voiceActors = readData("voiceActors");
...
const routes = [
  ...
  "/staff",
  ...staff.map((s2) => `/staff/${s2.id}`),
  "/studios",
  ...studios.map((s2) => `/studios/${s2.id}`),
  "/cast",
  ...voiceActors.map((v) => `/cast/${v.id}`),
  "/awards",
